...
public class PlaceAction {
    public static List<ISnapshot> getSnapshots(World world, PlayerEntity player, BlockHitResult rayTraceResult,
        ...
        int availableItems = countAvailableItems(player, wand, targetItem, world);
    ...
    private static int countAvailableItems(PlayerEntity player, ItemStack wand, BlockItem targetItem, World world) {
        if (player.isCreative()) return Integer.MAX_VALUE;

        int count = 0;

        if (ItemWand.hasPouch(wand)) {
            count += ItemWand.countInPouch(wand, targetItem, world);
        }
